...
# LangGraph is used for defining workflows, nodes, and edges in a state-based graph that represents different stages in the financial analysis process.
...
def research_competitors_node(state: AgentState):
    content = state.get("content", [])  # Use .get() to avoid KeyError if "content" is missing
    for competitor in state["competitors"]:
        # Invoke the model without structured output
        ...
        # Assuming the response contains queries in plain text
        queries = response.content.splitlines()  # Adjust depending on the structure of your output
...
def research_critique_node(state: AgentState):
    ...
    response = model.invoke(messages)
    queries = response.content.splitlines()  # Split the response into lines
    content = state.get("content", [])  # Retrieve or initialize content
    ...
    for q in queries:
        ...
        q = q.split('. ')[-1]  # This will remove numbering like '1. ', '2. ', etc.
        print(f"Querying Tavily with: '{q}'")  # Debug: Log the query being sent
        ...
        try:
            ...
            print(f"HTTP error occurred while querying Tavily: {e}")  # Handle the error gracefully
...
graph = builder.compile()  # Do not pass the checkpointer for now
...
def main():
    ...
    task = st.text_input(
        "Enter the task:",
        "Analyze the financial performance of our company (WillowTree) compared to competitors",
    )
